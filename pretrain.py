...
def loss_func(y_hat, trg, bos_len=1,pad_idx=1):
...
def __main__():
    ...
    DS_v = pre_val_Dataset
    ds_train = DS_t(outfall_name=out_fall_name)
    ds_val = DS_v(outfall_name=out_fall_name)
    ...
    saving_root = Path("./runs") / f'PRET_ml_new_{max_len}_padidx_{pad_idx}_bos_len_{bos_len}_lr_{learning_rate}_epochs_{n_epochs}_d_model_{d_model}_of_{out_fall_name}_RI_{Rainfall_intensity}'
